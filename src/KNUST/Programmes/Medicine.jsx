...
function Medicine() {
  ...
  const MED = {
      name: 'Bsc. Human Biology (Medicine)(MMEDB)',
      grade: 6,
...
if (MED.req.includes(val[0][0]) && MED.req.includes(val[0][1])) {
   console.log('Electives req included');
   elective = true;
} else if (MED.req.includes(val[0][0]) && MED.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (MED.req.includes(val[0][0]) && MED.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (MED.req.includes(val[0][1]) && MED.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (MED.req.includes(val[0][1]) && MED.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (MED.req.includes(val[0][2]) && MED.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme');
  ...
  if (elective == true){
  let biology = 0;
  if(val[0][0] == MED.main.biology){
     biology = oneGrade.substring(1)
    
  } else if(val[0][1] == MED.main.biology){
     biology = twoGrade.substring(1)
  } else if(val[0][2] == MED.main.biology){
  ...
  } else if(chemistry > MED.scores.chemistry) {
    mainState = false;
