...
function Physiotherapy() {
  ...
  const NUS= {
      name: 'Bsc. Physiotherapy and Sport Science',
      grade: 12,
  ...
  let name = NUS.name;
  let college = NUS.university;
  let elective = electives;


 if (NUS.electives.includes(val[0][0]) && NUS.electives.includes(val[0][1]) && NUS.electives.includes(val[0][2])) {
   console.log('Your electives are not in line with this programme ');
   elective = true;
} else if (NUS.electives.includes(val[0][0]) && NUS.electives.includes(val[0][1]) && NUS.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (NUS.electives.includes(val[0][0]) && NUS.electives.includes(val[0][2]) && NUS.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (NUS.electives.includes(val[0][1]) && NUS.electives.includes(val[0][2]) && NUS.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme ');
...
if (NUS.req.includes(val[0][0]) && NUS.req.includes(val[0][1])) {
   console.log('Electives req included');
   elective = true;
} else if (NUS.req.includes(val[0][0]) && NUS.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (NUS.req.includes(val[0][0]) && NUS.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (NUS.req.includes(val[0][1]) && NUS.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (NUS.req.includes(val[0][1]) && NUS.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (NUS.req.includes(val[0][2]) && NUS.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme');
  ...
  if (elective == true){
  let biology = 0;
  if(val[0][0] == NUS.main.biology){
     biology = oneGrade.substring(1)
    
  } else if(val[0][1] == NUS.main.biology){
     biology = twoGrade.substring(1)
  } else if(val[0][2] == NUS.main.biology){
  ...
  let chemistry = 0;
  if(val[0][0] == NUS.main.chemistry){
     chemistry = oneGrade.substring(1)
    
  } else if(val[0][1] == NUS.main.chemistry){
    chemistry = twoGrade.substring(1)
  } else if(val[0][2] == NUS.main.chemistry){
  ...
  } else if(chemistry > NUS.scores.chemistry) {
    mainState = false;
 ...
 if(mainState == true){
  ...
  console.log(array)
   ...
   if(myGrade > NUS.grade){
    console.log("Your grade is bad")
    if (!knust_failed.some(item => item.name === name)) {
      knust_failed.push({ name, college, reason:`Sorry, your aggregate is ${myGrade}. You need aggregate 12 or better ` });
  }
  }else{
