...
function HerbalMedicine() {
  ...
  let name =BHM.name;
  let college =BHM.university;
  let elective = electives;


 if (BHM.electives.includes(val[0][0]) && BHM.electives.includes(val[0][1]) &&BHM.electives.includes(val[0][2])) {
   console.log('Your electives are not in line with this programme ');
   elective = true;
} else if (BHM.electives.includes(val[0][0]) && BHM.electives.includes(val[0][1]) &&BHM.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (BHM.electives.includes(val[0][0]) && BHM.electives.includes(val[0][2]) &&BHM.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (BHM.electives.includes(val[0][1]) &&BHM.electives.includes(val[0][2]) &&BHM.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme ');
...
if (BHM.req.includes(val[0][0]) &&BHM.req.includes(val[0][1])) {
   console.log('Electives req included');
   elective = true;
} else if (BHM.req.includes(val[0][0]) &&BHM.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (BHM.req.includes(val[0][0]) &&BHM.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (BHM.req.includes(val[0][1]) &&BHM.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (BHM.req.includes(val[0][1]) &&BHM.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (BHM.req.includes(val[0][2]) &&BHM.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme');
  ...
  if (elective == true){
  let biology = 0;
  if(val[0][0] ==BHM.main.biology){
     biology = oneGrade.substring(1)
    
  } else if(val[0][1] ==BHM.main.biology){
     biology = twoGrade.substring(1)
  } else if(val[0][2] ==BHM.main.biology){
  ...
  let chemistry = 0;
  if(val[0][0] ==BHM.main.chemistry){
     chemistry = oneGrade.substring(1)
    
  } else if(val[0][1] ==BHM.main.chemistry){
    chemistry = twoGrade.substring(1)
  } else if(val[0][2] ==BHM.main.chemistry){
  ...
  } else if(chemistry >BHM.scores.chemistry) {
    mainState = false;
 ...
 if(mainState == true){
  ...
  console.log(array)
   ...
   if(myGrade >BHM.grade){
    console.log("Your grade is bad")
    if (!knust_failed.some(item => item.name === name)) {
      knust_failed.push({ name, college, reason:`Sorry, your aggregate is ${myGrade}. You need aggregate 13 or better ` });
  }
  }else{
