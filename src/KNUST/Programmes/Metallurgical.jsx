...
function PMetallurgicalEng() {
  ...
  const BCH = {
      name: 'Bsc. PMetallurgical Engineering',
      grade: 11,
 ...
 if (BCH.electives.includes(val[0][0]) && BCH.electives.includes(val[0][1]) && BCH.electives.includes(val[0][2])) {
   ...
   if (!knust_failed.some(item => item.name === name)) {
      knust_failed.push({ name, college, reason:"Your electives are not in line with this programme. You will need physics, elective maths and chemistry" });
  }
}
...
export default PMetallurgicalEng
