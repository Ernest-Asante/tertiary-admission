...
function Vertinary() {
  ...
  const VET = {
      name: 'Doctor of Vertinary Medicine',
      grade: 11,
      req:["biology","chemistry", "animal husbandry"],
      main:{
       biology:'biology',
       chemistry:'chemistry',
      ...
      university: "KNUST",
      electives: ["emath", "physics", "chemistry","biology", "animal husbandry"],
      scores:
      {
        physics: 6,
        chemistry: 6,
  ...
  let name =VET.name;
  let college =VET.university;
  let elective = electives;


 if (VET.electives.includes(val[0][0]) && VET.electives.includes(val[0][1]) &&VET.electives.includes(val[0][2])) {
   console.log('Your electives are not in line with this programme ');
   elective = true;
} else if (VET.electives.includes(val[0][0]) && VET.electives.includes(val[0][1]) &&VET.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (VET.electives.includes(val[0][0]) && VET.electives.includes(val[0][2]) &&VET.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (VET.electives.includes(val[0][1]) &&VET.electives.includes(val[0][2]) &&VET.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme ');
...
if (VET.req.includes(val[0][0]) &&VET.req.includes(val[0][1])) {
   console.log('Electives req included');
   elective = true;
} else if (VET.req.includes(val[0][0]) &&VET.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (VET.req.includes(val[0][0]) &&VET.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (VET.req.includes(val[0][1]) &&VET.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (VET.req.includes(val[0][1]) &&VET.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (VET.req.includes(val[0][2]) &&VET.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme');
  ...
  if (elective == true){
  let biology = 0;
  if(val[0][0] ==VET.main.biology){
     biology = oneGrade.substring(1)
    
  } else if(val[0][1] ==VET.main.biology){
     biology = twoGrade.substring(1)
  } else if(val[0][2] ==VET.main.biology){
     biology = threeGrade.substring(1)
  } else if(val[0][3] ==VET.main.biology){
  ...
  let chemistry = 0;
  if(val[0][0] ==VET.main.chemistry){
     chemistry = oneGrade.substring(1)
    
  } else if(val[0][1] ==VET.main.chemistry){
    chemistry = twoGrade.substring(1)
  } else if(val[0][2] ==VET.main.chemistry){
    chemistry = threeGrade.substring(1)
  } else if(val[0][3] ==VET.main.chemistry){
    chemistry = fourGrade.substring(1)
  } else{
    if (!knust_failed.some(item => item.name === name)) {
        knust_failed.push({ name, college, reason:"Your electives are not in line with this programme. You will need chemistry to qualify" });
    }
  }
  ...
  if(val[0][0] ==VET.main.animal){
     animal = oneGrade.substring(1)
    
  } else if(val[0][1] ==VET.main.animal){
     animal = twoGrade.substring(1)
  } else if(val[0][2] ==VET.main.animal){
     animal = threeGrade.substring(1)
  } else if(val[0][3] ==VET.main.animal){
     animal = fourGrade.substring(1)
  };
  ...
  } else if(chemistry >VET.scores.chemistry) {
    mainState = false;
...
if(selectedOption == "Agric Science" && animal > VET.scores.animal){
    if (!knust_failed.some(item => item.name === name)) {
        knust_failed.push({ name, college, reason:`Sorry, your animal husbandry grade is ${animal}. You need C6 or better ` });
    }
}
 ...
 if(mainState == true){
  ...
  let myGrade;
   if(selectedOption == "Agric Science"){ myGrade = Number(chemistry)+Number(animal)+ otherGrade+ Number(mathsGrade.substring(1)) + Number(englishGrade.substring(1)) + Number(scienceGrade.substring(1))}
   else{ myGrade = Number(chemistry)+Number(biology)+ otherGrade+ Number(mathsGrade.substring(1)) + Number(englishGrade.substring(1)) + Number(scienceGrade.substring(1))}
   console.log(myGrade)

   if(myGrade >VET.grade){
