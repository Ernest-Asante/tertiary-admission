...
function ForestReserve() {
  ...
  const FRT = {
      name: 'Bsc. Forest Reserve Technology',
      grade: 20,
  ...
  let name = FRT.name;
  let college = FRT.university;
  let elective = electives;


 if (FRT.electives.includes(val[0][0]) && FRT.electives.includes(val[0][1]) && FRT.electives.includes(val[0][2])) {
   console.log('Your electives are not in line with this programme ');
   elective = true;
} else if (FRT.electives.includes(val[0][0]) && FRT.electives.includes(val[0][1]) && FRT.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (FRT.electives.includes(val[0][0]) &&  FRT.electives.includes(val[0][2]) && FRT.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (FRT.electives.includes(val[0][1]) &&FRT.electives.includes(val[0][2]) &&FRT.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme ');
...
if (FRT.req.includes(val[0][0]) ||FRT.req.includes(val[0][1])) {
   console.log('Electives req included');
   elective = true;
} else if (FRT.req.includes(val[0][0]) ||FRT.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (FRT.req.includes(val[0][0]) ||FRT.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (FRT.req.includes(val[0][1]) ||FRT.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (FRT.req.includes(val[0][1]) ||FRT.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (FRT.req.includes(val[0][2]) ||FRT.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme');
  ...
  if (selectedOption === "General Science" ){
   ...
   if(val[0][0] ==FRT.main.biology){
      biology = oneGrade.substring(1)
     
   } else if(val[0][1] ==FRT.main.biology){
      biology = twoGrade.substring(1)
   } else if(val[0][2] ==FRT.main.biology){
  ...
  if (selectedOption === "Agric Science"){
   ...
   if(val[0][0] ==FRT.main.agric){
      agric = oneGrade.substring(1)
     
   } else if(val[0][1] ==FRT.main.agric){
      agric = twoGrade.substring(1)
   } else if(val[0][2] ==FRT.main.agric){
  ...
  let mainState = true;
    ...
    if (selectedOption === "General Science" && biology >FRT.scores.biology) {
     mainState = false;
   ...
   else if (selectedOption === "Agric Science" && agric >FRT.scores.agric) {
      mainState = false;
 ...
 if(mainState == true){
  ...
  console.log(sortedArr)
   ...
   if(myGrade >FRT.grade){
    console.log("Your grade is bad")
    if (!knust_failed.some(item => item.name === name)) {
      knust_failed.push({ name, college, reason:`Sorry, your aggregate is ${myGrade}. You need aggregate 20 or better ` });
  }
  }else{
