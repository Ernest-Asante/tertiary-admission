...
function MediLab() {
  ...
  let name = MLS.name;
  let college = MLS.university;
  let elective = electives;


 if (MLS.electives.includes(val[0][0]) && MLS.electives.includes(val[0][1]) && MLS.electives.includes(val[0][2])) {
   console.log('Your electives are not in line with this programme ');
   elective = true;
} else if (MLS.electives.includes(val[0][0]) && MLS.electives.includes(val[0][1]) && MLS.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (MLS.electives.includes(val[0][0]) && MLS.electives.includes(val[0][2]) && MLS.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (MLS.electives.includes(val[0][1]) && MLS.electives.includes(val[0][2]) && MLS.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme ');
...
if (MLS.req.includes(val[0][0]) && MLS.req.includes(val[0][1])) {
   console.log('Electives req included');
   elective = true;
} else if (MLS.req.includes(val[0][0]) && MLS.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (MLS.req.includes(val[0][0]) && MLS.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (MLS.req.includes(val[0][1]) && MLS.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (MLS.req.includes(val[0][1]) && MLS.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (MLS.req.includes(val[0][2]) && MLS.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme');
  ...
  if (elective == true){
  let biology = 0;
  if(val[0][0] == MLS.main.biology){
     biology = oneGrade.substring(1)
    
  } else if(val[0][1] == MLS.main.biology){
     biology = twoGrade.substring(1)
  } else if(val[0][2] == MLS.main.biology){
  ...
  let chemistry = 0;
  if(val[0][0] == MLS.main.chemistry){
     chemistry = oneGrade.substring(1)
    
  } else if(val[0][1] == MLS.main.chemistry){
    chemistry = twoGrade.substring(1)
  } else if(val[0][2] == MLS.main.chemistry){
  ...
  } else if(chemistry > MLS.scores.chemistry) {
    mainState = false;
