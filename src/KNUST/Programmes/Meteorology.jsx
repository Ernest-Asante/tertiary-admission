...
function Meteorology() {
  ...
  let name = MCS.name;
  let college = MCS.university;
  let elective = electives;


 if (MCS.electives.includes(val[0][0]) && MCS.electives.includes(val[0][1]) && MCS.electives.includes(val[0][2])) {
   console.log('Your electives are not in line with this programme ');
   elective = true;
} else if (MCS.electives.includes(val[0][0]) && MCS.electives.includes(val[0][1]) && MCS.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (MCS.electives.includes(val[0][0]) && MCS.electives.includes(val[0][2]) && MCS.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (MCS.electives.includes(val[0][1]) && MCS.electives.includes(val[0][2]) && MCS.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme ');
   ...
   if (!knust_failed.some(item => item.name === name)) {
      knust_failed.push({ name, college, reason:"Your electives are not in line with this programme. You will need physics, elective maths and chemistry or electronics" });
  }
}
...
if (MCS.req.includes(val[0][0]) && MCS.req.includes(val[0][1])) {
   console.log('Electives req included');
   elective = true;
} else if (MCS.req.includes(val[0][0]) && MCS.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (MCS.req.includes(val[0][0]) && MCS.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (MCS.req.includes(val[0][1]) && MCS.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (MCS.req.includes(val[0][1]) && MCS.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (MCS.req.includes(val[0][2]) && MCS.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme');
  ...
  if (elective == true){
  let physics = 0;
  if(val[0][0] == MCS.main.physics){
     physics = oneGrade.substring(1)
    
  } else if(val[0][1] == MCS.main.physics){
     physics = twoGrade.substring(1)
  } else if(val[0][2] == MCS.main.physics){
  ...
  let emath = 0;
  if(val[0][0] == MCS.main.emath){
     emath = oneGrade.substring(1)
    
  } else if(val[0][1] == MCS.main.emath){
     emath = twoGrade.substring(1)
  } else if(val[0][2] == MCS.main.emath){
  ...
  } else if(emath > MCS.scores.emath) {
    mainState = false;
