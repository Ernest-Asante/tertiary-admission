...
function WaterResourceManagement() {
  ...
  const WRM = {
      name: 'Bsc. Aquaculture and Water Resource Management',
      grade: 14,
  ...
  let name = WRM.name;
  let college = WRM.university;
  let elective = electives;


 if (WRM.electives.includes(val[0][0]) && WRM.electives.includes(val[0][1]) && WRM.electives.includes(val[0][2])) {
   console.log('Your electives are not in line with this programme ');
   elective = true;
} else if (WRM.electives.includes(val[0][0]) && WRM.electives.includes(val[0][1]) && WRM.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (WRM.electives.includes(val[0][0]) && WRM.electives.includes(val[0][2]) && WRM.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (WRM.electives.includes(val[0][1]) && WRM.electives.includes(val[0][2]) && WRM.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme ');
...
if (WRM.req.includes(val[0][0]) || WRM.req.includes(val[0][1])) {
   console.log('Electives req included');
   elective = true;
} else if (WRM.req.includes(val[0][0]) || WRM.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (WRM.req.includes(val[0][0]) || WRM.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (WRM.req.includes(val[0][1]) || WRM.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (WRM.req.includes(val[0][1]) || WRM.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (WRM.req.includes(val[0][2]) || WRM.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme');
  ...
  if (selectedOption === "General Science" ){
   ...
   if(val[0][0] == WRM.main.biology){
      biology = oneGrade.substring(1)
     
   } else if(val[0][1] == WRM.main.biology){
      biology = twoGrade.substring(1)
   } else if(val[0][2] == WRM.main.biology){
  ...
  if (selectedOption === "Agric Science"){
   ...
   if(val[0][0] == WRM.main.agric){
      agric = oneGrade.substring(1)
     
   } else if(val[0][1] == WRM.main.agric){
      agric = twoGrade.substring(1)
   } else if(val[0][2] == WRM.main.agric){
  ...
  let mainState = true;
    ...
    if (selectedOption === "General Science" && biology > WRM.scores.biology) {
     mainState = false;
   ...
   else if (selectedOption === "Agric Science" && agric > WRM.scores.agric) {
      mainState = false;
 ...
 if(mainState == true){
  ...
  console.log(sortedArr)
   ...
   if(myGrade > WRM.grade){
    console.log("Your grade is bad")
    if (!knust_failed.some(item => item.name === name)) {
      knust_failed.push({ name, college, reason:`Sorry, your aggregate is ${myGrade}. You need aggregate 19 or better ` });
  }
  }else{
