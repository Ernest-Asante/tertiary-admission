...
function Dietetics() {
  ...
  let name = DTC.name;
  let college = DTC.university;
  let elective = electives;


 if (DTC.electives.includes(val[0][0]) && DTC.electives.includes(val[0][1]) && DTC.electives.includes(val[0][2])) {
   console.log('Your electives are not in line with this programme ');
   elective = true;
} else if (DTC.electives.includes(val[0][0]) && DTC.electives.includes(val[0][1]) && DTC.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (DTC.electives.includes(val[0][0]) && DTC.electives.includes(val[0][2]) && DTC.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else if (DTC.electives.includes(val[0][1]) && DTC.electives.includes(val[0][2]) && DTC.electives.includes(val[0][3])) {
   console.log('Your electives are  in line with this programme ');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme ');
...
if (DTC.req.includes(val[0][0]) && DTC.req.includes(val[0][1])) {
   console.log('Electives req included');
   elective = true;
} else if (DTC.req.includes(val[0][0]) && DTC.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (DTC.req.includes(val[0][0]) && DTC.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (DTC.req.includes(val[0][1]) && DTC.req.includes(val[0][2])) {
   console.log('Electives req included');
   elective = true;
} else if (DTC.req.includes(val[0][1]) && DTC.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else if (DTC.req.includes(val[0][2]) && DTC.req.includes(val[0][3])) {
   console.log('Electives req included');
   elective = true;
} else {
   console.log('Your electives are not in line with this programme');
  ...
  if (elective == true){
  let biology = 0;
  if(val[0][0] == DTC.main.biology){
     biology = oneGrade.substring(1)
    
  } else if(val[0][1] == DTC.main.biology){
     biology = twoGrade.substring(1)
  } else if(val[0][2] == DTC.main.biology){
  ...
  let chemistry = 0;
  if(val[0][0] == DTC.main.chemistry){
     chemistry = oneGrade.substring(1)
    
  } else if(val[0][1] == DTC.main.chemistry){
    chemistry = twoGrade.substring(1)
  } else if(val[0][2] == DTC.main.chemistry){
  ...
  } else if(chemistry > DTC.scores.chemistry) {
    mainState = false;
