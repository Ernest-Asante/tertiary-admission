  ...
  return (
    ...
    <Card   sx={{minHeight:"88vh",marginBottom:1,margin:0,borderRadius:5}}> 
      <Box>
        ...
        {rank=="qualify"?(<p>PROGRAMS YOU QUALIFY FOR</p>):(<p>PROGRAMS YOU DO NOT QUALIFY FOR AT UG_LEGON</p>)}
        <ul> {ugcore_failed.map((ad, index) => (
        ...
        {rank=="qualify"?(
                ...
                <div>
                     {ug_passed.length > 0 ?( <ol>
                          {ug_passed.map((ad, index) => (
                                <li key={index}  style={{marginBottom:"7px"}}>
                                    <strong>Programme:</strong> {ad.name}, <strong>Institution:</strong> {ad.college}
                                </li>
                            ))}
                        </ol>):(<h3>YOU DO NOT QUALIFY FOR ANY PROGRAMME(DEGREE) AT UG_LEGON</h3>)} 
