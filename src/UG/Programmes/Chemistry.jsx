...
function Chemistry() {
 ...
 if (BCH.electives.includes(val[0][0]) && BCH.electives.includes(val[0][1]) && BCH.electives.includes(val[0][2])) {
   ...
   if (!ug_failed.some(item => item.name === name)) {
      ug_failed.push({ name, college, reason:"Your electives are not in line with this programme. You will need physics, elective maths and chemistry" });
  }
}

  console.log(elective)

  if(mathsGrade.substring(1) > 3){
    elective = false;
    if (!ug_failed.some(item => item.name === name)) {
        ug_failed.push({ name, college, reason:`Sorry, your emath grade is ${mathsGrade}. You need B3 or better ` });
    }
  }
  ...
  if (emath > BCH.scores.emath) {
    ...
    if (!ug_failed.some(item => item.name === name)) {
      ug_failed.push({ name, college, reason:`Sorry, your emath grade is ${emath}. You need B3 or better ` });
  }
